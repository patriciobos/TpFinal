...
void initLed (const led_t ledNumber)
{
	if (ledNumber < (sizeof (led) / sizeof (led_t))) {
		...
		/* Set pin */
		Board_LED_Set(ledNumber,true);
		/* Clear pin */
		Board_LED_Set(ledNumber,false);
	}
}
...
{
	if (ledNumber < (sizeof (led) / sizeof (led_t)))
		Chip_GPIO_SetPinState (LPC_GPIO_PORT, gpioLEDPins[ledNumber].gpio, gpioLEDPins[ledNumber].bit, (bool) !On);
...
{
	if (ledNumber < (sizeof (led) / sizeof (led_t)))
		return (bool) !Chip_GPIO_GetPinState(LPC_GPIO_PORT, gpioLEDPins[ledNumber].gpio, gpioLEDPins[ledNumber].bit);
//	if (LEDNumber < (sizeof(gpioLEDBits) / sizeof(io_port_t)))
