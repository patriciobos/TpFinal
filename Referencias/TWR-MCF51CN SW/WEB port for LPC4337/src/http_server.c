...
extern const uint8_t *InternalMemoryPointers[];
extern const uint16_t InternalMemoryLengths[];
extern const uint8_t number_of_files;
...
/*Read Buffer for App*/  
extern uint8_t *u8Archivo;
/*SDcard status*/
static uint8_t SD_present = FALSE;
 ...
 */
static uint8_t HTTP_ProcessConnection( web_session *connfd_web, uint8_t *space )
{
 ...
 * @return none
 */
static void HTTP_ProcessRequest(struct netconn *pcb, void *request, uint8_t *temporal_buffer, uint8_t flag)
{
     /*parsing variables*/
     char *end_temp;
     char *buffer;
     ...
     if( flag == GET_FLAG )
     {
        buffer = (char *)((uint32_t)q->payload + (uint32_t)(str_len(HTTP_GET_REQUEST)));
     }
     else if( flag == POST_FLAG )
     {
        buffer = (char *)((uint32_t)q->payload + (uint32_t)(str_len(HTTP_POST_REQUEST)));
     }
     ...
     if((end_temp = (char *)strchr((const char *)buffer,' ')) != NULL)
     {
          if((u16Length = (uint8_t)(end_temp - buffer)) != 0 )
          {
              ...
              if(flag == POST_FLAG)
              {
                 if( !CGI_parser((int8_t*)temporal_buffer,(void*)q) )
                 {
           	        //cgi not found
          	        HTTP_SendError(pcb);          	        
          	        return;                 
        	...
        	else//send DEFAULT WEB PAGE!!!!
        	{
        		  ...
        		  HTTP_SendWebPage(pcb, temporal_buffer, (uint8_t *)index_shtml, WEBPAGE_INDEX_LENGTH);
        	}
     }
     return;  
}
 ...
 */
static uint8_t HTTP_GetFileFromInternalMemory(struct netconn *pcb, uint8_t *name)
{
   uint8_t i;
   ...
   for(i=0;i<number_of_files;i++)
   {
      ...
      if(!strcmp(name,InternalMemoryFilenames[i]))
      {
          ...
          uint8_t *file_pointer = (uint8_t *)(InternalMemoryPointers[i]);
          uint16_t file_length = (uint16_t)InternalMemoryLengths[i];
...
{                                                   
    ...
    buffer = search_string_linked_list(string, u8Archivo, (uint8_t *)HTTP_CNTNT_LNG_REQUEST, (uint8_t *)STRING_END);
...
#endif          
     /*send header*/
     netconn_write(pcb, &array_to_send[0], (uint16_t)strlen(array_to_send), NETCONN_COPY );
}
...
HTTP_SendData(struct netconn *pcb, uint8_t dynamic_flag, uint8_t *array_to_send, uint8_t *dynamic_info, uint16_t dynamic_info_length)
{
    ...
    if( dynamic_flag ) 
    {
       HTTP_SendDynamicData(pcb, (uint8_t *)array_to_send, dynamic_info, dynamic_info_length, NULL/*send footer*/);
    }
    /*static process*/
    else
    {
       ...
       netconn_write(pcb, &dynamic_info[0], (uint16_t)dynamic_info_length, NULL);
#if HTTP_DYNAMIC_SUPPORT == 1
    }
#endif  
}
...
HTTP_SendDynamicData(struct netconn *pcb, uint8_t *array_to_send, uint8_t *dynamic_info, uint16_t dynamic_info_length, uint8_t flag)
{
     ...
     {
       if(( index_a = (uint8_t *)strstr(dynamic_start,SSI_START) ) != NULL)
       {
          if(( index_c = (uint8_t *)strstr(index_a,SSI_END) ) != NULL)
          {
              ...
              dynamic_length = dynamic_length -  (uint16_t)(dynamic_start - dynamic_start_temp);
          }
          else
          {
              //send static content
              HTTP_SendDynamicPacket(pcb, dynamic_start, ( uint16_t )dynamic_length, NULL);
              break;
          }
       }
       else
...
HTTP_Server_Task( void *pvParameters )
{
    ...
    uint8_t *temporal_buffer;

    /* Parameters are not used - suppress compiler error */
    ( void )pvParameters;
   
    /*Main temporal buffer*/
    if( (temporal_buffer=(uint8_t *)mem_malloc( HTTP_WORKING_SPACE )) == NULL )
    {
    ...
    else
    {
      ...
      if( (u8Archivo=(static uint8_t *)mem_malloc( BLOCK_SIZE+1 )) != NULL )
      {
#if HTTP_SD_CARD_SUPPORT 
        /*only tries to start sdcard in configuration mode, not in bridge mode*/
        if( !(uint8_t)board_get_bridge_configuration() )
        {
